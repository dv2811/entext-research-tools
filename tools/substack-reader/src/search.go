...
func runSearch(client *substack.Client, session *substack.Session, args []string) {
	...
	mode := fs.String("mode", "all", "Search mode: top, all, subscribed")
	page := fs.Int("page", 0, "Page number for pagination (0-10, not used for top mode)")
	language := fs.String("language", "", "Language code (2-letter, e.g., 'en')")
